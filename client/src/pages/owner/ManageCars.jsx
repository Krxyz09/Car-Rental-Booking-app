...
const ManageCars = () => {
  ...
  const toggleCarAvailability = async (carId) => {
    ...
    try {
      const confirm = window.confirm('Are you sure do you want to delete this car?')
            ...
            {car.map((car,index) => (
              <tr key={index} className='border-t border-borderColor'>
                ...
                <td className='p-3 max-md:hidden'>
                  <span>
                    {car.isAvailable ? <span className='px-3 py-0.5 bg-green-100 text-green-800 text-xs rounded-full'>Available</span> : <span className='px-3 py-0.5 bg-red-100 text-red-800 text-xs rounded-full'>Unavailable</span>}
                  </span>
                </td>
                <td className='flex items-center p-3'>
                  <img onClick={()=>toggleCarAvailability(car._id)} src={car.isAvailable ? assets.eye_close_icon : assets.eye_icon} alt="" className='cursor-pointer mr-3'  />
